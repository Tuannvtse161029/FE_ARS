/**
 * LandingScrollVideo — Five-act scroll-driven landing page
 *
 * Five-act narrative built on scrollcraft devices:
 *   Act 1 (parallax)  — Hero photo + three parallax layers + headline
 *   Act 2 (parallax)  — Tension scene with parallax chips + counter
 *   Act 3 (kinetic)   — Five editorial workflow stages
 *   Act 4 (pan)       — Horizontal pan through role-based workspaces
 *   Act 5 (recognition) — Medal tier ladder (RecognitionAct)
 *   Act 6 (flow)      — Boundaries, testimonial, FAQ, CTA
 *
 * Layout architecture (rewrite, 2026-09-13):
 * - Each act is a sibling block inside <main>. No nested sticky stages.
 * - Acts size by their own content + a min-height buffer for parallax/
 *   pan scroll range. The buffer is content-driven, not vh-driven, so
 *   DevTools opening or browser zoom never breaks the layout.
 * - React reads each act's ref via getBoundingClientRect on scroll and
 *   on ResizeObserver, and applies translateX / translateY to the act's
 *   direct children (parallax layers, pan rail). The page root paints
 *   --page-base continuously so no act can leak --app-canvas through.
 * - All five scroll behaviors (parallax hero/tension, kinetic turn,
 *   pan substance, spotlight CTA) and the editorial trace stay.
 */
...
// reset on every scroll pass made the page unreadable. We render
// directly without engine scrubbing and drive scroll effects with local
// React hooks.
// ──────────────────────────────────────────────────────────────────────

// ── safeMatchMedia ───────────────────────────────────────────
// Guards against environments without window.matchMedia (jsdom test envs,
// SSR, older browsers). Returns the MediaQueryList when available, or
// null so callers can pick a safe default without crashing.
...
// observer attaches, we mark it seen immediately. Otherwise the
...
const useCountUp = (target: number, durationMs = 1400) => {
  ...
  useEffect(() => {
    if (!seen) return;
    const reduce = safeMatchMedia('(prefers-reduced-motion: reduce)')?.matches ?? false;
    if (reduce) {
      setValue(target);
      return;
    }
...
// ── useActProgress ───────────────────────────────────────────────
// Returns 0..1 progress of an act's top-to-bottom scroll through the
// viewport. 0 when the act's top reaches the viewport top, 1 when the
// act's bottom reaches the viewport top. Used by parallax and pan
// drivers so they don't depend on a nested sticky stage.
//
// The progress is clamped at both ends so transforms don't snap when
// the act leaves view. ResizeObserver re-measures on viewport changes
// (DevTools open/close, browser zoom, OS scrollbar appearance) so the
// transform doesn't drift from the actual scroll position.
const useActProgress = (
  actRef: React.RefObject<HTMLElement | null>,
  opts: { mobileBreakpoint?: number; mobileGate?: boolean } = {},
) => {
  const { mobileBreakpoint = 640, mobileGate = false } = opts;
  ...
  useEffect(() => {
    ...
    const compute = () => {
      if (mobileGate && window.innerWidth <= mobileBreakpoint) {
        setProgress(0);
        return;
      }
      const rect = node.getBoundingClientRect();
  ...
  }, [actRef, mobileBreakpoint, mobileGate]);
  return progress;
};
...
// ── useRailOverflowVw ───────────────────────────────────────────
// Measures how much the substance rail overflows the available width
// (in vw) so the pan driver knows how far to translate. On narrow
// viewports the rail collapses to a vertical stack and overflow is 0.
const useRailOverflowVw = (railRef: React.RefObject<HTMLElement | null>) => {
...
const Spotlight = ({ className = '', children }: SpotlightProps) => {
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
    const reduced = safeMatchMedia('(prefers-reduced-motion: reduce)')?.matches ?? false;
    const fine = safeMatchMedia('(hover: hover) and (pointer: fine)')?.matches ?? false;
...
// ════════════════════════════════════════════════════════════════
// EDITORIAL TRACE — signature move
// A thin horizontal line fixed at the bottom edge of the viewport.
// Scroll position is the playhead; passing an act stamps a marker
// on the trace. This is the bespoke interaction that lives on this
// site alone.
// ════════════════════════════════════════════════════════════════
const TRACE_ACTS = [
...
const EditorialTrace = () => {
  ...
  return (
    <div className={styles.editorialTrace} aria-hidden="true">
...
const HeroAct = ({ t }: { t: Translator }) => {
  ...
  // Reads scroll progress 0..1 as the act traverses the viewport.
  const progress = useActProgress(actRef);
  ...
  useEffect(() => {
    ...
    // Differential parallax — photo drifts slowest (reads as "the
    // room"), foreground faster, near fastest. Copy/metrics/actions
    // drift opposite the photo so the photo's up-drift makes them
    // feel anchored.
    if (posterRef.current) {
      posterRef.current.style.transform = `scale(1.06) translate3d(0, ${(-progress * 22).toFixed(2)}px, 0)`;
    }
    if (foregroundRef.current) {
      foregroundRef.current.style.transform = `translate3d(0, ${(progress * 38).toFixed(2)}px, 0)`;
    }
    if (nearRef.current) {
      nearRef.current.style.transform = `translate3d(0, ${(-progress * 64).toFixed(2)}px, 0)`;
      nearRef.current.style.opacity = String(Math.max(0, 1 - progress * 1.4));
    }
    if (copyRef.current) {
      copyRef.current.style.transform = `translate3d(0, ${(progress * 8).toFixed(2)}px, 0)`;
    }
    if (metricsRef.current) {
      metricsRef.current.style.transform = `translate3d(0, ${(progress * 28).toFixed(2)}px, 0)`;
    }
    if (actionsRef.current) {
      actionsRef.current.style.transform = `translate3d(0, ${(progress * 48).toFixed(2)}px, 0)`;
    }
  }, [progress, reduced]);
  ...
  return (
    ...
    >
      <div className={styles.heroStage}>
        ...
        {/* Lamp light effect — dark mode only */}
        {isDarkMode && (
          <>
...
const HeroCounter = ({ end, suffix, label }: { end: number; suffix: string; label: string }) => {
...
// ════════════════════════════════════════════════════════════════
// ACT 2 — TENSION: The cost of scattered tools
//
// Beige parchment surface with floating tag labels (the "what's
// wrong" complaints) drifting through the section. No background
// image, no parallax depth planes — the section reads as a quiet
// editorial breather between the dark hero and the dark workflow.
// ════════════════════════════════════════════════════════════════
const TensionAct = ({ t }: { t: Translator }) => {
  const actRef = useRef<HTMLElement>(null);
...
// ════════════════════════════════════════════════════════════════
// ACT 3 — TURN: Five editorial workflow stages
// ════════════════════════════════════════════════════════════════
const TurnAct = ({ t }: { t: Translator }) => {
  return (
    <section
      id="turn"
    ...
    >
      ...
      <div className={styles.turnInner}>
        ...
        <ol className={styles.turnList}>
          {WORKFLOW.map((step) => (
            <li key={step.num} className={styles.turnItem}>
...
const SubstanceAct = ({ t }: { t: Translator }) => {
  const actRef = useRef<HTMLElement>(null);
  const railRef = useRef<HTMLDivElement>(null);
  const progress = useActProgress(actRef, { mobileGate: true });
  ...
  return (
    ...
    >
      ...
      <header className={styles.substanceHeader}>
        ...
        <h2 id="substance-title" className={styles.substanceTitle}>
          {t(
...
// ════════════════════════════════════════════════════════════════
// ACT 6 — COMMITMENT (flow + spotlight)
// ════════════════════════════════════════════════════════════════
const CommitmentSection = ({ t }: { t: Translator }) => {
  ...
  return (
    <div className={styles.commitment}>
      ...
      >
        <img
          className={styles.scene}
          src={testimonialScene}
          alt=""
          aria-hidden="true"
        />
        <div className={styles.testimonialScrim} aria-hidden="true" />
