/**
 * Enhanced Landing Page with ScrollCraft Integration
 * 
 * This component demonstrates scrollcraft integration with the ARS design system,
 * featuring scroll-driven video, pinned sections, parallax effects, and more.
 */
...
import styles from './LandingScrollCraft.module.css';

// Import scrollcraft integration
import {
  ScrollPage,
  useReducedMotion,
...
import { arsScrollThemes } from '../../scrollcraft/ars-tokens';
...
function ScrollCounterAnimated({ end, suffix = '', className = '' }: ScrollCounterProps) {
  const ref = useRef<HTMLSpanElement>(null);
  const [count, setCount] = useState(0);
  const hasAnimated = useIntersectionObserver(ref, { threshold: 0.5 });
...
function ParallaxSection({ children, rate = -0.2, className = '' }: ParallaxSectionProps) {
...
// ── Main Enhanced Landing Component ─────────────────────────────
export const LandingScrollCraft = () => {
  const t = useT();
  ...
  // Simulate mount completion for entrance animations
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  useEffect(() => {
    ...
    // Workflow list intersection observer
    ...
    // Flow constellation animation observer
  ...
  const workflow = [
    ...
    },
    {
      ...
      description: t('landing.workflowStep3Body', 'Eligible reviewers return recommendations within their authorized workspace.'),
    },
    {
      title: t('landing.workflowStep4Title', 'Decide'),
      description: t('landing.workflowStep4Body', 'Administrators make the final publication decision. Reviewer recommendations are not publication decisions.'),
    },
    {
      title: t('landing.workflowStep5Title', 'Discover'),
      description: t('landing.workflowStep5Body', 'Only approved public research becomes available through the research catalog.'),
    },
  ];
  ...
  const workspaces = [
    ...
    },
    {
      ...
      description: t('landing.workspaceReviewerBody', 'Manage eligible assignments and provide evaluations within the review process.'),
    },
    {
      icon: GraduationCap,
      role: 'lecturer',
      title: t('landing.workspaceLecturerTitle', 'Lecturer workspace'),
      description: t('landing.workspaceLecturerBody', 'Coordinate research groups, seminars, learning materials, and academic milestones.'),
    },
    {
      icon: Network,
      role: 'student',
      title: t('landing.workspaceStudentTitle', 'Graduate student workspace'),
      description: t('landing.workspaceStudentBody', 'Participate in research groups, reports, learning activity, and academic collaboration.'),
    },
  ];
  ...
  const faqs = [
    {
      ...
      answer: t('landing.faq1A', 'Academic Research System is a role-based academic platform for research discovery, paper submission and review, seminars, collaboration, and academic workspaces.'),
    },
    {
    ...
    },
    {
      ...
      answer: t('landing.faq3A', 'Only research that is both approved and public belongs in the research catalog. Internal review content is not public.'),
    },
  ];
  ...
  const constellationIcons = [Users, BookOpen, GraduationCap, Network, BookOpen, MessageSquare];

  return (
    <ScrollPage
      theme={arsScrollThemes.academicNavy}
      showProgress={false}
      showGrain={true}
      ...
      <div className={styles.page}>
        ...
        <header className={styles.header}>
          <div className={styles.headerInner}>
            ...
            <nav className={styles.navigation} aria-label={t('landing.navAria', 'Landing page navigation')}>
              <a href="#workflow" onClick={(e) => handleAnchorClick(e, 'workflow')}>
                {t('landing.navWorkflow', 'Editorial workflow')}
              </a>
              <a href="#workspaces" onClick={(e) => handleAnchorClick(e, 'workspaces')}>
                {t('landing.navWorkspaces', 'Workspaces')}
              </a>
              <a href="#boundaries" onClick={(e) => handleAnchorClick(e, 'boundaries')}>
        ...
        <main id="main-content">
          ...
          <section className={styles.hero} aria-labelledby="landing-title">
            <ParallaxSection rate={-0.15} className={styles.heroBg}>
              <div className={styles.heroBgGradient} />
            </ParallaxSection>
            
            <div className={styles.heroInner}>
              <div className={styles.heroCopy}>
                <p className={`${styles.issueLine} ${mounted ? styles.visible : ''}`}>
                  {t('landing.heroBadge', 'Trusted by Vietnamese researchers')}
                </p>
                <h1 id="landing-title" className={mounted ? styles.visible : ''}>
                  {t('landing.heroTitle', 'Where Vietnamese researchers share, review, and advance science together.')}
                </h1>
                <p className={`${styles.heroLead} ${mounted ? styles.visible : ''}`}>
                  {t('landing.heroSubtitle', 'Discover research, join structured peer review, organize academic seminars, and collaborate by role — all in one secure platform.')}
                </p>

                {/* Animated metrics */}
                <div className={`${styles.heroMetrics} ${mounted ? styles.visible : ''}`}>
                  {HERO_METRICS.map((metric, i) => (
                    <React.Fragment key={metric.key}>
                      {i > 0 && <span className={styles.metricDivider} aria-hidden="true" />}
                      <div className={styles.metricItem}>
                        <span className={styles.metricValue}>
                          <ScrollCounterAnimated 
                            end={parseInt(metric.value.replace(/[^0-9]/g, ''))} 
                            suffix="+"
                          />
                        </span>
                        <span className={styles.metricLabel}>{t(metric.key, metric.fallback)}</span>
                      </div>
                    </React.Fragment>
                  ))}
                </div>

                <div className={`${styles.heroActions} ${mounted ? styles.visible : ''}`}>
                  ...
                  <a className={styles.textLink} href="#workflow" onClick={(e) => handleAnchorClick(e, 'workflow')}>
                    {t('landing.ctaSecondary', 'View seminars')} <ArrowRight size={16} aria-hidden="true" />
                  </a>
                </div>
              </div>
              ...
              <div className={styles.heroVisual} aria-hidden="true">
                ...
                <div className={styles.heroConstellation}>
                  <svg className={styles.constellationCanvas} viewBox="0 0 340 180" preserveAspectRatio="xMidYMid meet">
                    <line x1={constellationNodes[0].cx} y1={constellationNodes[0].cy} x2={constellationNodes[1].cx} y2={constellationNodes[1].cy} />
                    <line x1={constellationNodes[1].cx} y1={constellationNodes[1].cy} x2={constellationNodes[2].cx} y2={constellationNodes[2].cy} />
                    <line x1={constellationNodes[2].cx} y1={constellationNodes[2].cy} x2={constellationNodes[3].cx} y2={constellationNodes[3].cy} />
                    <line x1={constellationNodes[3].cx} y1={constellationNodes[3].cy} x2={constellationNodes[4].cx} y2={constellationNodes[4].cy} />
                    <line x1={constellationNodes[4].cx} y1={constellationNodes[4].cy} x2={constellationNodes[5].cx} y2={constellationNodes[5].cy} />
                    <line x1={constellationNodes[2].cx} y1={constellationNodes[2].cy} x2={constellationNodes[4].cx} y2={constellationNodes[4].cy} />
                  </svg>

                  {constellationNodes.map((_, i) => {
                    const Icon = constellationIcons[i];
                    return (
                      <div key={i} className={`${styles.constellationNode} ${styles[`cn${i}`]}`}>
          ...
          {/* ── Statement ─────────────────────────────────────── */}
          <ScrollFlowReveal className={styles.statement}>
            <div>
              <p className={styles.issueLine}>
                {t('landing.statementKicker', 'The platform')}
              </p>
              <h2>
                {t('landing.statementTitle', 'Research deserves more than an upload destination.')}
              </h2>
            </div>
            <p>
          ...
          {/* ── Features ─────────────────────────────────────── */}
          <ScrollFlowReveal className={styles.featuresSection} stagger={80}>
            <div className={styles.featuresInner}>
              <div className={styles.featuresHeader}>
                ...
                <h2>
                  {t('landing.featuresTitle', 'Everything your research team needs')}
                </h2>
              </div>
              <div className={styles.featuresGrid}>
                {FEATURES.map((feature) => {
                  ...
                  return (
                    <article key={feature.titleKey} className={styles.featureItem} data-role={feature.role}>
          ...
          <section className={styles.workflowSection} id="workflow" aria-labelledby="workflow-title">
            ...
            <ol ref={workflowListRef} className={`${styles.workflowList} ${workflowListVisible ? styles.revealed : ''}`}>
              {workflow.map((step, index) => (
                <li key={step.title} className={styles.workflowStep} data-num={`0${index + 1}`} aria-label={step.title}>
                  <h3>{step.title}</h3>
            ...
            <aside ref={flowSectionRef} className={styles.publicationFlow} aria-label={t('landing.flowAria', 'Publication decision flow diagram')}>
                ...
                {PUBLICATION_NODES.map((node, index) => {
                  ...
                  return (
                    <div key={`${node.role}-${index}`} className={styles.flowNodeItem} data-role={node.role}>
                      ...
                      {node.noteKey && <span className={styles.flowNodeNote}>{t(node.noteKey, node.noteFallback ?? '')}</span>}
                      {index < PUBLICATION_NODES.length - 1 && (
                        <div className={`${styles.flowConnectorLine} ${flowConnectorsDrawn ? styles.drawn : ''}`} aria-hidden="true">
                          <span className={`${styles.flowDot} ${flowDotsAnimated ? styles.animated : ''}`} />
                        </div>
                      )}
                    </div>
                  );
          ...
          {/* ── Boundaries ───────────────────────────────────── */}
          <ScrollFlowReveal className={styles.boundariesSection} id="boundaries" aria-labelledby="boundaries-title">
            <div className={styles.boundariesInner}>
              <div>
                ...
                <h2 id="boundaries-title">
                  {t('landing.boundariesHeading', 'The catalog shows approved public work, not the private work behind it.')}
                </h2>
              </div>
              <ul>
          ...
          {/* ── Workspaces ───────────────────────────────────── */}
          <ScrollFlowReveal className={styles.workspacesSection} id="workspaces" aria-labelledby="workspaces-title">
            <div className={styles.sectionHeading}>
              <p className={styles.issueLine}>
                {t('landing.workspacesKicker', 'Role-specific work')}
              </p>
              <h2 id="workspaces-title">
                {t('landing.workspacesHeading', 'A shared platform with focused responsibilities.')}
              </h2>
            </div>
          ...
          {/* ── Testimonial ──────────────────────────────────── */}
          <ScrollFlowReveal className={styles.testimonialSection} aria-label="Researcher testimonial">
            <div className={styles.testimonialInner}>
              <span className={styles.testimonialMark} aria-hidden="true">"</span>
              <blockquote className={styles.testimonialQuote}>
                {t('landing.testimonialQuote', 'ARS cut our team review time in half — clear feedback, no scattered emails.')}
              </blockquote>
              <div className={styles.testimonialAttribution}>
                <span className={styles.testimonialName}>{t('landing.testimonialName', 'Dr. Nguyen Minh Anh')}</span>
                <span className={styles.testimonialRole}>{t('landing.testimonialRole', 'Research Lead, HCMUT')}</span>
              </div>
            </div>
          </ScrollFlowReveal>

          {/* ── FAQ ──────────────────────────────────────────── */}
          <ScrollFlowReveal className={styles.faqSection} aria-labelledby="faq-title">
            <div className={styles.faqIntro}>
              <p className={styles.issueLine}>{t('landing.faqKicker', 'Read before you enter')}</p>
              <h2 id="faq-title">{t('landing.faqHeading', 'Essential context for the ARS platform.')}</h2>
            </div>
...
export default LandingScrollCraft;
