/**
 * EditSubFieldModal — ARS Research Constellation
 *
 * Modal allowing Admin to edit an existing SubField's name,
 * associated MajorField, and description via PUT /api/SubField/{id}.
 */
...
import type { MajorField, SubFieldWithRubric } from '../../types/domain';
import dialogStyles from './AdminDialog.module.css';
import styles from './CreateSubFieldModal.module.css';

interface Props {
  subField: SubFieldWithRubric | null;
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const EditSubFieldModal = ({ subField, open, onClose, onSuccess }: Props) => {
  const { t } = useI18n();
  ...
  useEffect(() => {
    ...
    // Populate from selected subfield
    setMajorFieldId(subField.majorFieldId ?? '');
    setName(subField.name);
    setDescription(subField.description ?? '');
    setErrors({});
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await fieldService.updateSub(subField.subFieldId, {
      ...
      toast.success(
        t('admin.rubric.toast.updateSuccess', 'Sub-field updated successfully.'),
      );
      onSuccess();
      onClose();
    } catch (err: unknown) {
      ...
      const msg =
        ...
        t('admin.rubric.toast.updateError', 'Failed to update sub-field. Please try again.');
      toast.error(msg);
    } finally {
      setSaving(false);
    }
        ...
        <div className={dialogStyles.header}>
          <div>
            <h2 className={dialogStyles.title}>
              {t('admin.rubric.editModal.title', 'Edit Sub-field')}
            </h2>
            <p className={dialogStyles.subtitle}>ID: {subField.subFieldId}</p>
          </div>
          <button
            type="button"
...
export default EditSubFieldModal;
