...
import {
  ...
  Quote,
  PenTool,
  // Newly added icons (variety for Admin Medals picker)
  ScrollText,
  Library,
  NotebookPen,
  NotebookText,
  ...
  Unlock,
  KeySquare,
  // UI icons (already used elsewhere in this file)
  Plus,
  CheckCircle2,
  XCircle,
  Edit2,
...
export const LUCIDE_ICONS_LIST = [
  ...
  // ---- Discussion, community, collaboration ----
 ...
 *
 * Tokens used (defined in src/styles/ars-tokens.css):
 *   --tier-{x}-bg, --tier-{x}-border, --tier-{x}-icon
 *   --tier-{x}-ring-outer, --tier-{x}-ring-inner
 *   --tier-{x}-numeral-bg, --tier-{x}-numeral-fg
 *   --tier-{x}-icon-scale, --tier-{x}-halo
 */
...
export const SafeMedalBadge: React.FC<{
  ...
  // Tier-specific icon scale (48% → 60%). Pulled from a CSS variable so
  // it can be retuned in one place for both themes.
  ...
  return (
    <span
      ...
      title={alt || iconName}
    >
      {/* Outer ring — tier-specific stroke color + width */}
      <span
        aria-hidden="true"
        style={{
          position: 'absolute',
      ...
      />
      {/* Inner bevel — present for Silver / Gold / Platinum to add a
          second concentric ring; transparent on Bronze so its single
          ring reads as humble. The opacity is read from the
          `--tier-{x}-ring-inner` token so each tier can opt in/out. */}
      <span
        aria-hidden="true"
        style={{
          position: 'absolute',
          inset: '4px',
          borderRadius: '50%',
          // We can't use a CSS variable inside `border:` and have it
          // resolve to `transparent` correctly because the border is a
          // shorthand that always renders a colour. Instead we set the
          // colour directly via the variable and let the tier-specific
          // token decide whether the bevel is visible (Bronze passes
          // `transparent`, others pass a real colour).
          border: `2px solid var(--tier-${tierKey}-ring-inner, transparent)`,
        }}
      />
      {/* Disc — the actual medal face */}
      <span
        aria-hidden="true"
        style={{
          position: 'absolute',
      ...
      {/* Roman-numeral tier indicator — sits in the top-right corner so
          it doesn't fight the icon, and so the tier is readable even at
          a glance from across the page. */}
      <span
        aria-hidden="true"
        style={{
          position: 'absolute',
