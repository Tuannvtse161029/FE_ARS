...
const fetchLecturerRoster = async (
  ...
  try {
    ...
    if (Array.isArray(colleagues) && colleagues.length > 0) {
      ...
      return { rows, outcome: { kind: 'empty' } as const };
    }
  } catch {
    // If colleagues endpoint not available, fallback to user roster below
  }
...
export const LecturerMaterialsPage = () => {
  ...
  const resolveColleagueName = (
    ...
    // 1. Direct name from BE response if available
                ...
                >
                  ...
                  {!isSharedFromColleague && isConfirming && !disabledDelete && (
                    <div className={styles.materialDeleteConfirm}>
                      <div className={styles.materialDeleteConfirmHeader}>
                      ...
                      {isCurrentlyShared && (
                        <div className={styles.materialDeleteShareWarning}>
                          <AlertTriangle size={13} aria-hidden />
                          <span>
                            {t(
                              'lecturer.materials.action.deleteSharedWarning',
                              'This material is currently shared with {count} colleague(s). Deleting it will permanently revoke their access.',
                              { count: activeShares.length },
                            )}
                          </span>
                        </div>
...
<<<<<<< HEAD
        <div className={styles.sharedTableCard}>
          ...
          >
            ...
            >
              {t(
                'lecturer.materials.shared.colHeader.material',
                'Material',
              )}
            </span>
            <span
            ...
            >
              {t(
                'lecturer.materials.shared.colHeader.expires',
                'Expires',
              )}
            </span>
            <span
            ...
            >
              {t(
                'lecturer.materials.shared.colHeader.status',
                'Status',
              )}
            </span>
            <span
            ...
            >
              {t(
                'lecturer.materials.shared.colHeader.actions',
                'Actions',
              )}
            </span>
          </div>
            ...
            {items.map((item) => {
              ...
              const targetId =
                typeof item.learningMaterialId === 'number'
                  ...
                  : null;
              const foundMaterial =
                targetId !== null && learningById
                  ? learningById.get(targetId)
                  : null;
              const openUrl =
                item.learningMaterialUrl ||
                item.fileUrl ||
              ...
              const statusVariantClass =
                styles[`sharedRowStatus${uiStatus}`] ?? '';
              const statusPillClass = statusVariantClass
                ? `${styles.sharedRowStatusPill} ${statusVariantClass}`
                : styles.sharedRowStatusPill;
              return (
                <li
                  key={String(id)}
