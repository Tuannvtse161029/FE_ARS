...
export const ResearcherSubmissionDetail = () => {
  ...
  return <section className={shared.page}><header className={shared.header}><div><h1>{paper.title}</h1><p>Private researcher submission view.</p></div><span className={shared.status}>{statusLabel(paper.status)}</span></header><div className={shared.panel}><div className={shared.actions}><CitationActions paper={paper} />{buildSafeResourceLink(paper.fileUrl) && <a className={shared.buttonSecondary} href={buildSafeResourceLink(paper.fileUrl) ?? undefined} target="_blank" rel="noopener noreferrer">Read PDF</a>}</div><section aria-label="Submission timeline"><h2>Submission timeline</h2><ol className={shared.timeline}>{timelineStatuses.map((status, index) => <li className={index <= currentIndex ? shared.timelineComplete : shared.timelinePending} key={status}><span aria-hidden="true" className={shared.timelineDot} />{statusLabel(status)}</li>)}</ol></section><p><strong>Version:</strong> {paper.version ?? 'Not supplied'}</p><p><strong>Authors:</strong> {paper.authors.map((author) => author.name).join(', ') || 'Not supplied'}</p><p><strong>Institution:</strong> {paper.institutions.map((item) => item.name).join(', ') || 'Not supplied'}</p><p><strong>Assigned reviewer:</strong> {publicReviewerName(paper) ?? (paper.reviewer ? 'Assigned reviewer (identity private)' : 'Not assigned by Admin')}</p><p><strong>Editorial feedback:</strong> {paper.researcherFeedback ?? 'No researcher feedback has been released.'}</p>{paper.status === 'PUBLISHED' && <p><strong>Publication notification:</strong> Your paper is published.</p>}</div></section>;
};
