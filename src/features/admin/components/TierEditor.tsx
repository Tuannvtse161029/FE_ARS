...
export interface TierEditorProps {
  ...
  locale: string;
  /**
   * Set of `criteriaMetric` codes already used by other active medals.
   * Optional — when not supplied, no metrics are considered "in use"
   * and the guard is a no-op. The parent (`AdminMedals`) computes this
   * from `medalService.getAll()` for the create / edit modal.
   */
  existingMetrics?: ReadonlySet<string>;
}
...
export const TierEditor = ({
  ...
  locale,
  existingMetrics = new Set<string>(),
}: TierEditorProps) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Reject duplicate metrics. The metric dropdown already disables used
    // codes, but a defence-in-depth guard here catches race conditions
    // (admin opens two editor tabs at once) and any future API mismatch.
    // In edit mode we allow the currently-edited medal's metric to remain.
    const isEditingCurrentMetric =
      mode === 'edit' && medal?.criteriaMetric === metricCode.trim();
    if (!isEditingCurrentMetric && existingMetrics.has(metricCode.trim())) {
      const errorMsg = copy(
        'This measurement metric is already used by another medal. Please pick a different one.',
        'Chỉ số đo lường này đã được dùng cho huy hiệu khác. Vui lòng chọn chỉ số khác.',
      );
      showNotification(errorMsg, 'error');
      setTitleError(errorMsg);
      return;
                ...
                <div className={styles.metricDropdownGroup}>
                  ...
                  >
                    ...
                    {PREDEFINED_METRICS.map((pm) => {
                      // While editing, the current medal's metric stays
                      // selectable so the admin can keep the same metric
                      // without seeing it greyed out.
                      const inUse = existingMetrics.has(pm.metric);
                      const isCurrent = mode === 'edit' && medal?.criteriaMetric === pm.metric;
                      const disabled = inUse && !isCurrent;
                      const label = locale === 'vi' ? pm.labelVi : pm.labelEn;
                      const inUseSuffix = locale === 'vi' ? ' (đã dùng)' : ' (in use)';
                      return (
                        <option key={pm.metric} value={pm.metric} disabled={disabled}>
                          {label}
                          {disabled ? inUseSuffix : ''}
                        </option>
                      );
                    })}
