/**
 * BoldArtworkPicker — admin-facing tile grid for the "Bold designs" tab of
 * the artwork modal. It surfaces the curated `BADGE_BOLD_ARTWORK_LIST`
 * subset of `BADGE_ARTWORK_REGISTRY` (gradient / glow / illustrative
 * pieces) so admins can pick something more dramatic than the flat
 * library without crowding a single picker.
 *
 * The UX mirrors `BadgeArtworkPicker` so both tabs feel native: same live
 * search, same grouped sections, same canonical URL emission. The only
 * difference is the slightly larger tile, the gradient frame, and the
 * "BOLD" hover badge — cues that signal the artwork is illustrative
 * rather than flat.
 */
...
import styles from './BoldArtworkPicker.module.css';

export interface BoldArtworkPickerProps {
...
export const BoldArtworkPicker = ({
  ...
  const searchPlaceholder = t(
    'admin.medals.bold.searchPlaceholder',
    locale === 'vi' ? 'Tìm trong bộ cá tính...' : 'Search bold designs...',
  );
  const searchAriaLabel = t(
    'admin.medals.bold.searchAria',
    locale === 'vi' ? 'Tìm biểu tượng cá tính' : 'Search bold designs',
  );
  const clearAriaLabel = t(
    'admin.medals.bold.clearAria',
    locale === 'vi' ? 'Xóa tìm kiếm' : 'Clear search',
  );
  const emptyTemplate = t(
    'admin.medals.bold.empty',
    locale === 'vi'
      ? 'Không có hình nào khớp "{query}"'
      : 'No bold design matches "{query}"',
  );
  const gridLabel = t(
    'admin.medals.bold.gridAria',
    locale === 'vi' ? 'Thư viện thiết kế cá tính' : 'Bold designs library',
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return BADGE_BOLD_ARTWORK_LIST;
    return BADGE_BOLD_ARTWORK_LIST.filter((entry) => {
      const haystack =
    ...
    <div className={styles.wrapper}>
      ...
      >
        {filtered.length === 0
          ...
          : `${filtered.length} ${locale === 'vi' ? 'hình' : 'designs'}`}
      </span>
...
export default BoldArtworkPicker;
